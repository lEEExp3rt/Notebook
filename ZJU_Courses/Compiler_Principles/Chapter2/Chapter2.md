...
通过上述方式构造的DFA其实不是最小的状态，当中存在某些状态可以进行合并

如果两个状态，一个状态能接收的字符串另一个状态也能接收，反过来不能接收的另一个也不能接收，那么这两个状态等价

> [!tip] 即两个状态能接收的字符串集合和不能接收的字符串集合等价

- 两个状态都是终止状态或都是非终止状态
- 对于任意符号 $c$，两个状态的状态转移 $\text{trans}[S_1,c]=\text{trans}[S_2,c]$ 一致

> [!warning] 这只是一个充分条件
> ![Exception](assets/MinimalDFAException.png)

称两个状态 $S_1$ 和 $S_2$ 是可区分的(*Distinguishable*)，如果存在一个终结符 $x$ 使得其中一个状态接收 $x$ 而另一个状态拒绝 $x$

如果两个状态是不是可区分的，那么它们就是等价的

> [!success] 所有的死状态都是等价的

DFA最小化就是把可区分的状态划分为不同的组

算法流程：

1. 初始化，把所有终结状态划分为一组，非终结状态划分为另一组
2. 开始迭代，把所有可能的输入应用于各个组
    - 只有一个组中的所有状态在读入同一字符后都落入同一个集合，那么这个组在这次迭代中不用继续分割
    - 否则，落入不同集合的状态要被进一步划分为新的组
3. 不断重复步骤2直到不动点，即任一组都不再能被进一步划分
4. 此时的等价类中的每个集合都对应于最小DFA的一个状态，可以基于此轻松构建最小化的DFA

> [!example] DFA最小化
> ![Example](assets/DFAMinimizationExamples.png)

## Lex

词法分析器生成器就是从词法规范生成一个词法分析器程序的程序，因为前文所述的构造DFA的过程非常机械，可以直接由计算机实现
